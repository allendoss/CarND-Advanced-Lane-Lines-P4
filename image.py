...
for idx, fname in enumerate(images):
    ...
    # Drawing the results
    template = np.array(rPoints+lPoints,np.uint8) # add left, right window pixels together
    ...
    middle_marker=np.array(list(zip(np.concatenate((right_fitx-windowWidth/2,right_fitx[::-1]+windowWidth/2),axis=0),np.concatenate((yvals,yvals[::-1]),axis=0))),np.int32)
    ...
    write_name = 'D:/SDCND/Project_4/CarND-Advanced-Lane-Lines/test_images/tracked' + str(idx) +'.jpg'
